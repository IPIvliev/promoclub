...
namespace :csv do
  ...
  task :create_promos => :environment do
    ...
    CSV.foreach(csv_file_path, :col_sep => ";", :row_sep => :auto, :headers => true) do |row|
      if row["name"] != nil && row["surname"] != nil && row["birth"] != nil && row["gender"] != nil && row["country_id"] != nil && row["city_id"] != nil && row["phone"] != nil && row["email"] != nil && row["med"] != nil && row["avatar"] != nil
        user = User.new({
        :name => row["name"],
        :surname => row["surname"],
        :birth => row["birth"],
